...
func (s *S) TestFieldMapBasic(c *gc.C) {
	fields := map[string]schema.Checker{
		"a": schema.Const("A"),
		"b": schema.Const("B"),
		"c": schema.Const("C"),
	}
...
func (s *S) TestFieldMapDependency(c *gc.C) {
	fields := map[string]schema.Checker{
		"a": schema.String(),
		"b": schema.String(),
	}
	defaults := map[string]interface{}{"b": schema.Omit}
	deps := map[string]schema.Dependency{
		"a": schema.Dependency{
			DependsOn: "b",
			Value:     "bbb",
		},
	}
	sch := schema.Schema{Checkers: fields, Defaults: defaults, Dependencies: deps}

	// b exists with the correct value, so a must exist.
	out, err := sch.Coerce(map[string]interface{}{"a": "a", "b": "bbb"}, aPath)
	c.Assert(err, gc.IsNil)
	c.Assert(out, gc.DeepEquals, map[string]interface{}{"a": "a", "b": "bbb"})

	// neither a nor b exist, that's ok. Note that a does not have a default,
	// but it still can be excluded if b doesn't have the correct value.
	out, err = sch.Coerce(map[string]interface{}{}, aPath)
	c.Assert(err, gc.IsNil)
	c.Assert(out, gc.DeepEquals, map[string]interface{}{})

	// b exists with the wrong value, so a must not exist.
	out, err = sch.Coerce(map[string]interface{}{"b": "ccc"}, aPath)
	c.Assert(err, gc.IsNil)
	c.Assert(out, gc.DeepEquals, map[string]interface{}{"b": "ccc"})

	// b exists with the wrong value, so a shouldn't exist, but it does.
	out, err = sch.Coerce(map[string]interface{}{"a": "a", "b": "ccc"}, aPath)
	c.Assert(err, gc.ErrorMatches, `field "a" should not be specified when "b" is ccc. "a" requires "b" to have value bbb`)
	c.Assert(out, gc.IsNil)

	// b exists with the correct value, so a must exist, but it does not.
	out, err = sch.Coerce(map[string]interface{}{"b": "bbb"}, aPath)
	c.Assert(err, gc.ErrorMatches, `field "b" exists with value bbb, but required field "a" is missing`)
	c.Assert(out, gc.IsNil)

	// a exists but its dependency b does not.
	out, err = sch.Coerce(map[string]interface{}{"a": "a"}, aPath)
	c.Assert(err, gc.ErrorMatches, `field "a" requires value bbb be specified for "b", but it is not specified`)
	c.Assert(out, gc.IsNil)

}

func (s *S) TestSchemaMap(c *gc.C) {
	fields1 := schema.FieldMap(map[string]schema.Checker{
		"type": schema.Const(1),
		"a":    schema.Const(2),
	}, nil)
	fields2 := schema.FieldMap(map[string]schema.Checker{
		"type": schema.Const(3),
		"b":    schema.Const(4),
	}, nil)
